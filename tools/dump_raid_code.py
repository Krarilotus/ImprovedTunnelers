...
tunnellers can march with their troop - and, next to it, the code behind the main army,
where a tunneller in an attack slot already takes the AI's orders.

usage: python tools/dump_raid_code.py "<path to Stronghold Crusader.exe>" [out.txt]

The default output is tools/raid-code.txt, inside the repository, so it can be committed
and pushed as it is.
...
ROOTS = [
    ...
    # The main army, where a tunneller in an attack slot does take its orders.
    'assignUnitToATribe', 'getUnitTypeForUnitBehaviourType', 'createTribeForUnitType',
    'smallestTribeOfUnitType', 'randomlySelectAttackUnitTypeToRecruit',
    'updateTribeBehaviorBasedOnBehaviorType', 'decideAndExecuteTribeAttackAction',
    'aiRegisterTribeAndAssignTarget', 'decideOnTribeAttackLocation',
    'setTargetForTribeBasedOnTargetChoiceLogic', 'checksAndGenerateAITribesForPlayerIfNotExisting',
    'clearOutdatedAITribes', 'spawnUnitAndAddToTribe', 'addUnitToNewTribe',
    # Raid sizes and the raid's own state, and where a new tunneller waits.
    'getCurrentDesiredAttackRaidUnitCount', 'setCurrentAttackRaidParameter', 'updateAIBehaviour',
    'UpdateTunnelersGuild', 'findFreeTunnelerAssemblyTile',
...
def main():
    if len(sys.argv) < 2:
        ...
        return 2
    path = sys.argv[1]
    out_path = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, 'raid-code.txt')
    ...
    # ... and the whole of every function that makes that test, whether or not it is a root.
    for c in callers:
        owner = max((s for s in starts if s <= c), default=None)
        if owner is not None and owner not in done:
            done.add(owner)
            order.append((owner, disassemble(owner)[0]))
    ...
    with open(out_path, 'w') as out:
        ...
        out.write('# calls of isTribeFreeOfTunnelingUnits: %s\n\n' % ', '.join(
            '0x%X (in %s)' % (c, names.get(max((s for s in starts if s <= c), default=0), '?'))
            for c in callers))
